...
class Stream extends AbstractWriter
{
    ...
    /**
     * Retreives the logged errors from the stream
     *
     *@var null
     */
    private $events;
    ...
    /**
     * Get logged events in the stream or file 
     * @edited  this method is added only by me 
     * @author Rabiu mustapha
     * @return array   $logged data in file or stream or empty array on failure
     */
    public function getErrorEvents(){
        //if the $stream file is not a resource, just write to it //
        if( !is_resource( $this->stream ) ){
            $this->events = file_get_contents( $this->stream )  ?: []; 
        }
        elseif( is_resource( $this->stream ) ){
            rewind( $this->stream ); //rewind back to begining of stream //
            $this->events = stream_get_contents( $this->stream )  ?: [];
        }

        return $this->formatter->deformat( $this->events ); //deformat the event to a readable version //
